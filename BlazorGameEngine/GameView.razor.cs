...
namespace BlazorGameEngine
{
    ...
    public partial class GameView(IJSRuntime js, IEnumerable<IGameAsset> assets)
    {
        ...
        private readonly Lazy<Task<IJSObjectReference>> moduleTask = new(() => js.InvokeAsync<IJSObjectReference>("import", $"/_content/{typeof(Game).Namespace}/js/gameinterop.js").AsTask());
        ...
        public async Task OnAnimationFrameAsync()
        {
            ...
            // only temporarily... This is in order to have the viewport update when something changes
            // currently handled via css left/top properties on the canvas container
            StateHasChanged();
        }
